...
{
	Students newStu = malloc(sizeof(struct _students));
	// figure out how many students
	newStu->nstu = 0;
	for (int i = 0; i < ss->nstu; i++) {
		if (ss->recs[i].wam > minWAM) newStu->nstu++;
	}
	newStu->recs = malloc(newStu->nstu * sizeof(sturec_t));
	for (int i = 0, j = 0; i < ss->nstu; i++) {
		if (ss->recs[i].wam > minWAM) {
			newStu->recs[j] = ss->recs[i];
			j++;
		}
	}

	return newStu; // TODO ... replace this line by your code
}
